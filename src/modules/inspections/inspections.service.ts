...
interface HeaderField       { key: string; label: string; required: boolean; }
interface MediaFieldSlot    { key: string; type: string; }
interface HealthTier        { minScore: number; maxScore: number; remainingLifeMinYears: number; remainingLifeMaxYears: number; }
interface HealthThreshold   { good: HealthTier; fair: HealthTier; poor: HealthTier; }
interface RepairConfig      { statuses: string[]; }
...
export class InspectionService {
  ...
  async submitInspection(
    ...
    const { dashboard, criteria } = await this._getCriteriaForDashboard(dashboardId);
    ...
    if (!activeSchedule)
      throw new NotFoundException(`Scheduled inspection "${scheduledInspectionId}" not found.`);
    if (activeSchedule.dashboardId !== dashboardId)
      throw new BadRequestException('This scheduled inspection does not belong to the given dashboard.');
    if (activeSchedule.assignedTo !== inspectorId)
      throw new ForbiddenException('This scheduled inspection is not assigned to you.');
    if (activeSchedule.status === ScheduledInspectionStatus.COMPLETE)
      throw new BadRequestException('This scheduled inspection has already been completed.');
    if (activeSchedule.status === ScheduledInspectionStatus.ASSIGNED)
      throw new BadRequestException('You must start the inspection before submitting. Call the /start endpoint first.');
    if (activeSchedule.status === ScheduledInspectionStatus.DUE)
      throw new BadRequestException('This scheduled inspection is overdue. Contact your admin to reschedule.');
    if (activeSchedule.status !== ScheduledInspectionStatus.IN_PROGRESS)
      throw new BadRequestException('Scheduled inspection is not in a valid state for submission.');

    const headerFields  = criteria.headerFields          as unknown as HeaderField[];
    const categories    = criteria.scoringCategories     as unknown as ScoringCategory[];
    const mediaSlots    = criteria.mediaFields           as unknown as MediaFieldSlot[];
    const repairConfig  = criteria.repairPlanningConfig  as unknown as RepairConfig;
    const thresholds    = criteria.healthThresholdConfig as unknown as HealthThreshold;

    // ── Validations ───────────────────────────────────────────────────────────
    for (const field of headerFields) {
      if (field.required && !dto.headerData?.[field.key])
        throw new BadRequestException(`Required header field "${field.label}" is missing.`);
    }

    for (const category of categories) {
      const submitted = dto.scores?.[category.key];
      if (submitted !== undefined && submitted.score > category.maxPoints)
        throw new BadRequestException(`Score for "${category.label}" is ${submitted.score} — max allowed is ${category.maxPoints}.`);
    }

    const { statuses } = repairConfig;
    for (const item of dto.repairItems ?? []) {
      if (!statuses.includes(item.status))
        throw new BadRequestException(`Repair status "${item.status}" is invalid. Allowed: ${statuses.join(', ')}.`);
    }

    const validSlotKeys = mediaSlots.map((s) => s.key);
    for (const key of dto.mediaFieldKeys ?? []) {
      if (!validSlotKeys.includes(key))
        throw new BadRequestException(`mediaFieldKey "${key}" does not exist in criteria.mediaFields.`);
    }

    // ── Compute score + health ────────────────────────────────────────────────
    const overallScore = categories.reduce(
      (sum, cat) => sum + (dto.scores?.[cat.key]?.score ?? 0), 0,
    );
    ...
    for (let i = 0; i < (files ?? []).length; i++) {
      ...
      const mediaFile = await this.prisma.mediaFile.create({
        data: {
          ...
          mediaFieldKey,
          category:
            slot?.type === 'embed'       ? 'tour'
            : mediaFieldKey === 'aerialMap' ? 'aerial'
    ...
    await this.prisma.scheduledInspection.update({
      ...
      data:  { status: ScheduledInspectionStatus.COMPLETE, inspectionId: inspection.id },
    });
    ...
    const accesses = await this.prisma.propertyAccess.findMany({
      where:  { propertyId: dashboard.property?.id, revokedAt: null, OR: [{ expiresAt: null }, { expiresAt: { gt: new Date() } }] },
      select: { userId: true },
    });
    if (accesses.length) {
    ...
    return {
      ...
      data:    { ...inspection, mediaFiles, summary: { overallScore, healthLabel, remainingLife } },
    };
  }
  ...
  async findAllForDashboard(dashboardId: string) {
    ...
    const inspections = await this.prisma.inspection.findMany({
      ...
      include: {
        ...
        mediaFiles: { select: { id: true, fileName: true, fileType: true, url: true, mediaFieldKey: true } },
      },
      orderBy: { createdAt: 'desc' },
    });

    return { success: true, message: 'Inspections retrieved', data: inspections };
  }
  ...
  async getAssignedInspections(
    ...
    const [scheduled, total] = await this.prisma.$transaction([
      this.prisma.scheduledInspection.findMany({
        ...
        include: {
          dashboard: {
            include: {
              property: { select: { name: true, address: true, propertyType: true } },
            },
          },
        },
      }),
    ...
    return {
      ...
      meta: { total, page, limit, total_pages, has_next_page: page < total_pages, has_prev_page: page > 1 },
    };
  }
  ...
  async getAllScheduled(filters: {
    ...
    const { status, assignedTo, dashboardId, dateFrom, dateTo, search, page, limit } = filters;
    const skip = (page - 1) * limit;
  ...
  async getOneScheduled(scheduledInspectionId: string) {
    ...
    if (!scheduled) throw new NotFoundException('Scheduled inspection not found.');
  ...
  async startInspection(scheduledInspectionId: string, operationalUserId: string) {
    const scheduled = await this.prisma.scheduledInspection.findUnique({
      where: { id: scheduledInspectionId },
    });
  ...
  private async _assertDashboardExists(dashboardId: string) {
    ...
    if (!dashboard)
      throw new NotFoundException(`PropertyDashboard "${dashboardId}" not found.`);
    return dashboard;
  }
  ...
  private async _getCriteriaForDashboard(dashboardId: string) {
    ...
    if (!criteria)
      throw new BadRequestException('This property has no active template or inspection criteria configured.');

    return { dashboard, criteria };
  }

  private _resolveFileType(mimetype: string): 'PHOTO' | 'VIDEO' | 'PDF' | 'EMBED' {
